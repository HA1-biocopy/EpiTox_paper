...
PieChart(binder, data = epitox, hole = 0,
         fill = biocopy_colors,
         color="white",
         main = paste0("Off-targets HLA-allele binding categories ", nrow(epitox)),
         labels = "input",
         labels_position= "in",
         labels_size=1.5,
...
epitox = openxlsx::read.xlsx("../../data/full_peptide_rankings.xlsx")
 ...
 PieChart(anchor_status, data = epitox, hole = 0,
             fill = c( "#958BB2", "#FBB800","#99CFE9", "#438D99"),
             color="white",
             main = paste0("anchor_status: ", nrow(epitox)),
             labels = "input",
             labels_position= "in",
             labels_size=1.5,
             labels_color = c("black", "white"),
