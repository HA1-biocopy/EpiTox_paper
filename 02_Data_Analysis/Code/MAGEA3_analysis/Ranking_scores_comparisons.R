...
bayesian = openxlsx::read.xlsx("/Users/hoor.alhasani/Documents/Projects/D003/Patent_Paper/paper_materials/02_Data_Analysis/data/Bayesian/MAGEA3_full_results.xlsx") %>%
  ...
  merge(., df[, c("id", "Rank", "Ranking_score")], by = "id", all = T)
...
# Create the table
conf_table <- bayesian %>%
  select(Rank, confidence_level) %>%
  table()

# Perform chi-square test
chisq_result <- chisq.test(conf_table)
print(chisq_result)

# Look at standardized residuals to see where the association is strongest
round(chisq_result$stdres, 2)

# Calculate row proportions
prop_table <- prop.table(conf_table, margin = 1)
print(round(prop_table, 3))

 as.data.frame(prop_table) %>%
  mutate(Rank = factor(Rank, levels = c("High", "Moderate", "Low", "Random"))) %>%
  ggplot(aes(x = Rank, y = Freq, fill = confidence_level)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  labs(y = "Proportion",
       title = "Experimental Evidenc Distribution by Bi-feature ranking",
       fill = "Experimental evidence") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = c("High" = "#D14B47", "Medium" = "#99CFE9", "Low" = "#FBB800", "Very Low" = "#A2C510"))
...
score_reference = openxlsx::read.xlsx("/Volumes/lab/03_HighSCORE/P048/P048_01_Results Triplicates/TTP-230_new/PPB-68_kinetic_table.xlsx") %>%
  ...
  filter (lev_dist <5, n>3, !peptide %in% c("KVADLIHFL", "TVAELVQFV")) %>% # first peptide is the QC droped one, the second is the incorrect one (TVAELVQFL)
  select(labguru_id, peptide, KD, n, lev_dist, Outcome, log2_target_FC, n_slides_QC_passed) %>%
  mutate(
    OT = ifelse(peptide %in% xscan$peptide, "Xscan", "OT"),
    Outcome =
...
ggplot(df, aes(confidence_level, KD)) +
  geom_boxplot(aes(fill = Bi_feature_rank), position = "dodge") +
  geom_jitter(aes(fill = Bi_feature_rank), shape = 21) +
  labs(x = "Bayesian weight probability") +
  theme_light() +
  facet_wrap(~Outcome) +
...
ggplot(df, aes(confidence_level, KD*10^5)) +
  ...
  facet_wrap(~Bi_feature_rank, scales = "free_x")
...
DIR = "/Users/hoor.alhasani/Documents/Projects/MAGEA3/lab/"
files = list.files(path = DIR, pattern = "*.csv", full.names = T)

ab = lapply(files, function(x){
  x = read.table(x,sep = ";") %>%
    t() #%>%
    # data.frame() %>%
    # colnames(x) = x[1,]
