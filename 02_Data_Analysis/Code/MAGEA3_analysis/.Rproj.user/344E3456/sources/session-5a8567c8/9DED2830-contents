...
bayesian_peptide_assessment <- function(peptide_data,
  ...
  if (is.null(likelihood_ratios)) {
    likelihood_ratios <- list(
      ...
      one_study = 10,           # Strong: 2 studies
      one_database = 5,         # Moderate: 1 study
...
generate_assessment_report <- function(assessed_peptides,
  ...
  # 2. Full results
  write.csv(assessed_peptides,
            paste0(output_prefix, "_full_results.csv"),
            row.names = FALSE)
  ...
  write.csv(high_priority,
            paste0(output_prefix, "_high_priority.csv"),
            row.names = FALSE)
